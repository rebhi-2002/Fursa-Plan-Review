...
export default function JobsPage() {
  ...
  const { data: jobsResponse, isLoading } = useListJobs({
    ...
    type: (type as ListJobsType) || undefined,
    limit: 50
  });
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="text-muted-foreground">
            تصفح أحدث الفرص الوظيفية المتاحة في غزة
          </p>
        </div>
        ...
        <Card className="bg-muted/30 border-dashed">
          <CardContent className="p-4 flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              <Search className="absolute right-3 top-3 h-4 w-4 text-muted-foreground rtl:left-auto rtl:right-3 ltr:left-3 ltr:right-auto" />
              <Input 
        ...
        <div className="flex flex-col gap-4">
          <div className="text-sm text-muted-foreground">
            {isLoading ? "جاري البحث..." : `تم العثور على ${jobsResponse?.total || 0} وظيفة`}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              jobsResponse.items.map((job) => (
                <Card key={job.id} className="flex flex-col h-full hover:shadow-md transition-shadow border-border/50">
              ...
              <div className="col-span-full py-16 text-center bg-muted/20 rounded-2xl border border-dashed">
                ...
                <h3 className="text-lg font-medium">لا توجد وظائف مطابقة</h3>
                <p className="text-muted-foreground mt-1">جرب تغيير كلمات البحث أو التصنيفات.</p>
                <Button 
                  variant="link" 
